...
Aim
To interface an external push button with an STM32 microcontroller and control the state of an LED based on the push-button
input.
Apparatus Required
S. No.
Component
...
: main.c
* @brief          
: Main program body (Blink LED only on button hold)
******************************************************************************
*/
...
#endif
The push button was successfully interfaced with the STM32 microcontroller. The LED connected to PA5 turned ON when the
push button connected to PA0 was pressed (logic HIGH) and turned OFF when the push button was released (logic LOW).
